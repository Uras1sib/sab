--[[
    VORTEX LABS - AUTO-CLOSE GUI v12.2
]]
...
local VORTEX_DATA = {
    ...
    Modes = {
        ...
        [99606176102979] = "https://api.luarmor.net/files/v4/loaders/e8c2b7bdfd494b913839f58581a203f9.lua",
    },
    Theme = {
...
local function Notify(title, msg, color)
    ...
    n.Position = UDim2.new(1.2, 0, 0.85, 0)
    ...
    TweenService:Create(n, TweenInfo.new(0.6, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Position = UDim2.new(0.75, 0, 0.85, 0)}):Play()
    task.spawn(function()
        task.wait(4)
        local out = TweenService:Create(n, TweenInfo.new(0.6, Enum.EasingStyle.Back, Enum.EasingDirection.In), {Position = UDim2.new(1.2, 0, 0.85, 0)})
        out:Play()
        out.Completed:Wait()
        n:Destroy()
    end)
...
local function LaunchVortex()
    ...
    Instance.new("UIStroke", Main).Color = VORTEX_DATA.Theme.Accent

    local Title = Instance.new("TextLabel", Main)
    Title.Size = UDim2.new(1, 0, 0, 70); Title.Text = VORTEX_DATA.Name; Title.TextColor3 = VORTEX_DATA.Theme.Accent; Title.Font = Enum.Font.GothamBold; Title.TextSize = 24; Title.BackgroundTransparency = 1

    local K_Frame = Instance.new("Frame", Main)
    K_Frame.Size = UDim2.new(0.85, 0, 0, 50); K_Frame.Position = UDim2.new(0.075, 0, 0.42, 0); K_Frame.BackgroundColor3 = Color3.fromRGB(20, 20, 26); Instance.new("UICorner", K_Frame)
    local K_Input = Instance.new("TextBox", K_Frame)
    K_Input.Size = UDim2.new(1, -20, 1, 0); K_Input.Position = UDim2.new(0, 10, 0, 0); K_Input.PlaceholderText = "Erişim Anahtarını Girin..."; K_Input.Text = ""; K_Input.TextColor3 = Color3.new(1, 1, 1); K_Input.BackgroundTransparency = 1; K_Input.Font = Enum.Font.GothamMedium

    local function CreateButton(txt, pos, color, callback)
        local btn = Instance.new("TextButton", Main)
        btn.Size = UDim2.new(0.4, 0, 0, 45); btn.Position = pos; btn.Text = txt; btn.BackgroundColor3 = color; btn.TextColor3 = Color3.new(1, 1, 1); btn.Font = Enum.Font.GothamBold; Instance.new("UICorner", btn)
    ...
    CreateButton("GİRİŞ YAP", UDim2.new(0.075, 0, 0.68, 0), VORTEX_DATA.Theme.Accent, function()
        ...
        if success and inputKey ~= "" then
            ...
            if keyFound then
                local scriptToLoad = VORTEX_DATA.Modes[game.PlaceId]
                if scriptToLoad then
                    Notify("ONAYLANDI", "Yükleniyor...", Color3.fromRGB(0, 255, 120))
                    CloseUI()
                    loadstring(game:HttpGet(scriptToLoad))()
                else
                    Notify("HATA", "Oyun ID tanımlanmamış. Panel kapatılıyor.", Color3.fromRGB(255, 50, 50))
                    task.wait(1)
                    CloseUI()
                end
            else
                Notify("GEÇERSİZ", "Yanlış anahtar! Panel kapatılıyor.", Color3.fromRGB(255, 50, 50))
                task.wait(1)
                CloseUI()
            end
        else
            Notify("HATA", "Bağlantı hatası! Panel kapatılıyor.", Color3.fromRGB(255, 50, 50))
            task.wait(1)
            CloseUI()
        end
    end)
    ...
    CreateButton("DISCORD", UDim2.new(0.525, 0, 0.68, 0), Color3.fromRGB(88, 101, 242), function()
        ...
        Notify("KOPYALANDI", "Discord linki kopyalandı.", Color3.fromRGB(88, 101, 242))
    end)
end
